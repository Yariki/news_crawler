...
async def test_remove_role_from_user_success(client, create_user, create_role, db_session):
    ...
    response_assign_role = await client.post(f"/admin/users/{user_id}/roles", json={"roles_ids": role_ids})
    assert response_assign_role.status_code == 200
    
    # Act
    
    response = await client.delete(f"/admin/users/{user_id}/roles/{role_ids[0]}")
    
    # Assert
    assert response.status_code == 200

    user = (await db_session.execute(select(User).where(User.id == user_id).options(selectinload(User.roles)))).scalar_one()

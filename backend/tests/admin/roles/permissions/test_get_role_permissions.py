...
async def test_get_role_permission_success(
    client: AsyncClient,
    ...
    response_permissions = await client.get(f"/admin/roles/{role_id}/permissions")

    # Assert
    assert response_permissions.status_code == 200
    data = response_permissions.json()
    assert isinstance(data, list)
    assert len(data) > 0


async def test_get_role_permission_failed_role_not_found(
    client: AsyncClient,
    create_role: Callable[..., RoleCreateUpdate],
    create_permission: Callable[..., PermissionCreateUpdate],
    db_session: AsyncSession,
    ...
    # Arrange
    role_id = '0af9fc88-f028-4a26-820a-91a4d7d0232a'
    ...
    # Assert
    assert response_permissions.status_code == 404

        ...
        try:
            ...
            for url in urls:
                ...
                for keyword in matched_keywords:
                    self.db.add(KeywordHit(
                        article_id=article.id, keyword=keyword))
            ...
            logger.error(
                "Database error during crawl job for source %s: %s", source_id, db_exc)
            job.status = Status.FAILED
            job.error_message = "Database error occurred during crawling."
        except httpx.HTTPStatusError as http_exc:
            logger.error(
                "HTTP error during crawl job for source %s: %s", source_id, http_exc)
            job.status = Status.FAILED
            job.error_message = f"HTTP error {http_exc.response.status_code} while crawling."
        except httpx.RequestError as req_exc:
            logger.error(
                "Network error during crawl job for source %s: %s", source_id, req_exc)
            job.status = Status.FAILED
            job.error_message = "Network error occurred during crawling."
        except Exception as exc:
            logger.error(
                "Unexpected error during crawl job for source %s: %s", source_id, exc)
            job.status = Status.FAILED

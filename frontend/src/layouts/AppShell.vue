        ...
        <div class="pa-4">
          <v-card rounded="xl" variant="tonal">
            ...
            <v-list lines="two" density="compact">
              <v-list-item v-for="(alert, index) in store.alerts.slice(0, 5)" :key="index" :href="alert.url" target="_blank">
                <v-list-item-title>{{ alert.title }}</v-list-item-title>
                <v-list-item-subtitle>{{ (alert.matched_keywords || []).join(', ') }}</v-list-item-subtitle>
              </v-list-item>
